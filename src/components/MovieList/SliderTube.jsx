              ...
              {item.key ? (
                ...
                <Link href={`/movie/${item.id}`} key={index} className="cursor-pointer" passHref>
                  <div className="">
                    ...
                    <div className="w-full pl-4 sm:pl-12 pt-4 sm:pt-12 sm:w-1/2">
                      <h1 className="text-2xl sm:text-5xl pb-2 text-white font-bold ">{item.title}</h1>

                      <h1 className="py-6 pr-4 text-md sm:text-xl text-white ">{item.overview.slice(0, 170)}...</h1>
                      <side className="flex flex-row">
                        <button className="mr-4 bg-red-600 text-lg rounded-xl border text-color-primary border-color-primary sm:p-2 p-1 bg-color-secondary">
                          <h1 className="text-xl sm:text-3xl text-white">{item.release_date.slice(0, 4)}</h1>
                        </button>

                        <button className="bg-red-600 text-lg rounded-xl border text-color-primary border-color-primary sm:p-2 p-1 bg-color-secondary">
